        ...
        <CardContent>
          <Tabs defaultValue="preview" className="w-full">
            <TabsList className="inline-flex sticky top-0 z-10 bg-background border-b mb-4">
              ...
              {article.showNotes && (
                <TabsTrigger value="shownotes" className="gap-2">
            ...
            {article.showNotes && (
              <TabsContent value="shownotes" className="min-h-[400px]">
                <div className="space-y-6">
                  {/* Chapters */}
                  <div className="border rounded-lg p-4">
                    <div className="flex items-center justify-between mb-4">
                      <h3 className="font-semibold text-lg">Kapitelmarken</h3>
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => {
                          const chaptersText = article.showNotes!.chapters
                            .map(c => `${c.timestamp} - ${c.title}\n${c.description}`)
                            .join("\n\n");
                          copyToClipboard(chaptersText, "Kapitelmarken");
                        }}
                      >
                        <Copy className="h-4 w-4 mr-2" />
                        Alle kopieren
                    ...
                    <div className="space-y-3">
                      {article.showNotes.chapters.map((chapter, idx) => (
                        <div key={idx} className="flex gap-4 p-3 bg-muted rounded">
                          <div className="font-mono text-sm font-semibold text-primary min-w-[60px]">
                            {chapter.timestamp}
                          </div>
                          <div className="flex-1">
                  ...
                  {/* Quotes */}
                  {article.showNotes.quotes.length > 0 && (
                    <div className="border rounded-lg p-4">
                      <div className="flex items-center justify-between mb-4">
                        <h3 className="font-semibold text-lg">Zitate & Soundbites</h3>
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => copyToClipboard(article.showNotes!.quotes.join("\n\n"), "Zitate")}
                        >
                          <Copy className="h-4 w-4 mr-2" />
                          Alle kopieren
                        </Button>
                      ...
                      <div className="space-y-3">
                        {article.showNotes.quotes.map((quote, idx) => (
                          <div key={idx} className="relative p-4 bg-muted rounded border-l-4 border-primary">
                            <p className="text-sm italic">&ldquo;{quote}&rdquo;</p>
                            <Button
                              variant="ghost"
                              size="sm"
                              className="absolute top-2 right-2"
                              onClick={() => copyToClipboard(quote, "Zitat")}
                            >
                              <Copy className="h-3 w-3" />
                            </Button>
                          </div>
                  ...
                  {/* Resources */}
                  {article.showNotes.resources.length > 0 && (
                    <div className="border rounded-lg p-4">
                      <div className="flex items-center justify-between mb-4">
                        <h3 className="font-semibold text-lg">Erwähnte Ressourcen</h3>
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => copyToClipboard(article.showNotes!.resources.join("\n"), "Ressourcen")}
                        >
                          <Copy className="h-4 w-4 mr-2" />
                          Alle kopieren
                        </Button>
                      ...
                      <ul className="space-y-2">
                        {article.showNotes.resources.map((resource, idx) => (
                          <li key={idx} className="flex items-center gap-2 p-2 bg-muted rounded">
                            ...
                            <Button
                              variant="ghost"
                              size="sm"
                              onClick={() => copyToClipboard(resource, "Ressource")}
                            >
                              <Copy className="h-3 w-3" />
                            </Button>
                          </li>
                  ...
                  {/* Guest Bio */}
                  {article.showNotes.guests && article.showNotes.guests.trim() !== "" && (
                    <div className="border rounded-lg p-4">
                      <div className="flex items-center justify-between mb-3">
                        <h3 className="font-semibold text-lg">Gast-Information</h3>
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => copyToClipboard(article.showNotes!.guests, "Gast-Bio")}
                        >
                          <Copy className="h-4 w-4" />
                        </Button>
                      </div>
